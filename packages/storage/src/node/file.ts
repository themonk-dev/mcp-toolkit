...
import { createEncryptor, type Encryptor } from '../crypto/aes-gcm.ts';
import type {
  ProviderTokens,
  RsRecord,
  TokenStore,
...
import { type MemoryTokenSnapshot, MemoryTokenStore } from '../memory.ts';
...
/**
 * On-disk shape. `records[]` carries the `expiresAt` augmentation in addition
 * to the public `RsRecord` fields — historically that field was leaked into
 * JSON by `JSON.stringify` over `rsAccessMap.values()`, so it's part of the
 * de-facto file format and existing `.data/tokens.json` files depend on it.
 *
 * Older files (pre-snapshot API) that lack `expiresAt` on individual records
 * are still tolerated: load() backfills with `provider.expires_at` or the
 * default RS-token TTL.
 */
type PersistShape = {
  version: number;
  encrypted: boolean;
  records: Array<RsRecord & { expiresAt?: number }>;
};
...
export class FileTokenStore implements TokenStore {
  ...
  private async loadAsync(): Promise<void> {
    ...
    try {
      ...
      // Build a snapshot the MemoryTokenStore can restore from. Older files
      // may not carry `expiresAt` on each record (it was leaked into JSON by
      // the previous implementation but never officially part of the shape),
      // so backfill from `provider.expires_at` or the default RS-token TTL.
      const snapshot: MemoryTokenSnapshot = {
        rsRecords: validRecords.map((rec) => ({
          ...rec,
          expiresAt:
            rec.expiresAt ?? rec.provider.expires_at ?? now + 7 * 24 * 60 * 60 * 1000,
        })),
      };
  ...
  private async saveAsync(): Promise<void> {
    ...
    try {
      ...
      // Get all records via the snapshot API. The returned `rsRecords` array
      // carries the `expiresAt` augmentation, matching the pre-existing on-disk
      // shape (which stringified the augmented map values directly).
      const snapshot = this.memory.snapshot();
      const data: PersistShape = {
        version: 1,
        encrypted: Boolean(this.encryptor),
        records: snapshot.rsRecords,
      };
      ...
      logger.debug('file_token_store', {
        message: 'File saved',
        records: snapshot.rsRecords.length,
        encrypted: Boolean(this.encryptor),
      });
    } catch (error) {
      logger.error('file_token_store', {
