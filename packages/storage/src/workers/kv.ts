...
export class KvTokenStore implements TokenStore {
  ...
  async saveCode(code: string, txnId: string, ttlSeconds = 600): Promise<void> {
    ...
    try {
      ...
      logger.warning('kv_token_store', {
        message: 'Failed to save code (using memory)',
        error: (error as Error).message,
      });
      // Don't throw - memory has it
    }
...
export class KvSessionStore implements SessionStore {
  ...
  async get(sessionId: string): Promise<SessionRecord | null> {
    ...
    this.putSession(sessionId, session).catch((error) =>
      logger.warning('kv_session_store', {
        message: 'Failed to update last_accessed on session',
        sessionId,
        error: (error as Error).message,
      }),
    );
